...
#ifdef CONFIG_ENCODERS
static void mpeg4_inv_pred_ac(MpegEncContext * s, DCTELEM *block, int n, int dir);
...
#if 0 //3IV1 is quite rare and slows things down a tiny bit
...
void h263_encode_picture_header(MpegEncContext * s, int picture_number)
{
    ...
    if (!s->h263_plus) {
        ...
        put_bits(&s->pb, 1, 0);	/* Advanced Prediction: off */
        ...
        put_bits(&s->pb, 3, s->pict_type == P_TYPE); /* only I/P frames, no PB frame */
...
{
    align_put_bits(&s->pb); // GSTUFF
    put_bits(&s->pb, 17, 1); // GBSC
    s->gob_number = s->mb_y / s->gob_index;
...
//#define DEBUG 1
#ifdef DEBUG
    printf("Encode GOB header at MB %d, GN %d (0,%d), GFID %d, GQUANT %d\n",
           (s->mb_y * s->mb_width) + s->mb_x, s->gob_number, s->mb_y, (s->pict_type == I_TYPE), s->qscale);
#endif
}

#define int_32to7(a)	((a) & 0x7F)
#define compl_2(a)	(-(a))

/**
 * Send a video packet.
 */
void h263_send_video_packet(MpegEncContext * s)
{
    uint8_t *hdr = pbBufPtr(&s->rtp_pb);
    uint8_t *data = s->rtp_last_ptr;
    int hdr_size, data_size;

//printf("send_video_packet %d,%d ->] %d,%d\n", s->rtp_last_mb_x, s->rtp_last_mb_y, s->mb_x, s->mb_y);

    if(s->flags&CODEC_FLAG_RFC2190){
        if(s->flags&CODEC_FLAG_MODE_A_ONLY){

            put_bits(&s->rtp_pb, 1, 0); // mode A
            put_bits(&s->rtp_pb, 1, !!s->max_b_frames); // pb
            put_bits(&s->rtp_pb, 3, s->rtp_last_sbit);
            put_bits(&s->rtp_pb, 3, s->rtp_gob_ebit);
            put_bits(&s->rtp_pb, 3, h263_get_picture_format(s->width, s->height));
            put_bits(&s->rtp_pb, 1, (s->pict_type == P_TYPE));
            put_bits(&s->rtp_pb, 1, s->umvplus);
            put_bits(&s->rtp_pb, 1, 0); // sac
            put_bits(&s->rtp_pb, 1, 0); // ap
            put_bits(&s->rtp_pb, 4, 0); // reserved
            if(s->max_b_frames){
                printf("unknown values");
            }else{
                put_bits(&s->rtp_pb, 2, 0); // dbq
                put_bits(&s->rtp_pb, 3, 0); // trb
                put_bits(&s->rtp_pb, 8, 0); // tr
            }

//printf("sbit %d, ebit %d, from 0x%.8x to 0x%.8x (now is 0x%.8x at %d:%d)\n", s->rtp_last_sbit, s->rtp_gob_ebit, (int)data, (int)s->rtp_gob_ptr, (int)pbBufPtr(&s->pb), s->mb_x, s->mb_y);

            data_size = s->rtp_gob_ptr - data;
            if(s->rtp_gob_ebit) data_size++;

            s->rtp_last_ptr = s->rtp_gob_ptr;
            s->rtp_last_sbit = (- s->rtp_gob_ebit) & 7;
        }else{
            if(s->rtp_last_mb_x == 0 && s->rtp_last_mb_y % s->gob_index == 0){

                put_bits(&s->rtp_pb, 1, 0); // mode A
                put_bits(&s->rtp_pb, 1, !!s->max_b_frames); // pb
                put_bits(&s->rtp_pb, 3, s->rtp_last_sbit);
                put_bits(&s->rtp_pb, 3, pbBufEndBit(&s->pb));
                put_bits(&s->rtp_pb, 3, h263_get_picture_format(s->width, s->height));
                put_bits(&s->rtp_pb, 1, (s->pict_type == P_TYPE));
                put_bits(&s->rtp_pb, 1, s->umvplus);
                put_bits(&s->rtp_pb, 1, 0); // sac
...
//printf("q %d, gobn %d, mba %d, mv0 %d:%d (%d:%d)\n",
//       s->rtp_last_qscale, s->rtp_last_mb_y / s->gob_index, s->rtp_last_mb_x + (s->rtp_last_mb_y % s->gob_index)*s->mb_width, s->rtp_last_mv0_x, s->rtp_last_mv0_y, int_32to7(compl_2(s->rtp_last_mv0_x)), int_32to7(compl_2(s->rtp_last_mv0_y)));

                put_bits(&s->rtp_pb, 1, 1); // mode B/C
                put_bits(&s->rtp_pb, 1, !!s->max_b_frames); // pb
                put_bits(&s->rtp_pb, 3, s->rtp_last_sbit);
                put_bits(&s->rtp_pb, 3, pbBufEndBit(&s->pb));
                put_bits(&s->rtp_pb, 3, h263_get_picture_format(s->width, s->height));
                put_bits(&s->rtp_pb, 5, s->rtp_last_qscale);
                put_bits(&s->rtp_pb, 5, s->rtp_last_mb_y / s->gob_index);
                put_bits(&s->rtp_pb, 9, s->rtp_last_mb_x + (s->rtp_last_mb_y % s->gob_index)*s->mb_width);
                put_bits(&s->rtp_pb, 2, 0); // reserved
                put_bits(&s->rtp_pb, 1, (s->pict_type == P_TYPE));
                put_bits(&s->rtp_pb, 1, s->umvplus);
                put_bits(&s->rtp_pb, 1, 0); // sac
                put_bits(&s->rtp_pb, 1, 0); // ap
                put_bits(&s->rtp_pb, 7, int_32to7(compl_2(s->rtp_last_mv0_x)));
                put_bits(&s->rtp_pb, 7, int_32to7(compl_2(s->rtp_last_mv0_y)));
                put_bits(&s->rtp_pb, 7, int_32to7(compl_2(s->rtp_last_mv3_x)));
                put_bits(&s->rtp_pb, 7, int_32to7(compl_2(s->rtp_last_mv3_y)));
                if(s->max_b_frames){
                    printf("unknown values\n");
                }
            }

            data_size = pbBufPtr(&s->pb) - data;
            if(pbBufEndBit(&s->pb)) data_size++;

            s->rtp_last_ptr = pbBufPtr(&s->pb);
            s->rtp_last_sbit = (- pbBufEndBit(&s->pb)) & 7;
            s->rtp_last_mb_x = s->mb_x;
            s->rtp_last_mb_y = s->mb_y;

            if(s->mb_x || s->mb_y % s->gob_index){
                s->rtp_last_qscale = s->qscale;
                s->rtp_save_mv = 1;
            }
        }
    }else{
    ...
    align_put_bits(&s->rtp_pb); // sanity check

    hdr_size = pbBufPtr(&s->rtp_pb) - hdr;

//printf("hdr %d (from %.8x) data %d (from %.8x)\n", hdr_size, (int)hdr, data_size, (int)data);

    if(s->rtp_callback)
        s->rtp_callback(data, data_size, hdr, hdr_size, s->opaque);
}

/**
 * Send a video packet.
 */
void rtp_send_video_packet(MpegEncContext * s)
{
    int data_size;

    data_size = pbBufPtr(&s->pb) - s->rtp_last_ptr;
    if(pbBufEndBit(&s->pb)) data_size++;

    if(s->rtp_callback)
        s->rtp_callback(s->rtp_last_ptr, data_size, NULL, 0, s->opaque);
...
//                s->mb_skiped=1;
			...
			diff= s->dsp.pix_abs16x16(p_pic, b_pic, s->linesize);
                ...
                if(s->mb_skiped==1){
                    put_bits(&s->pb, 1, 1); /* mb not coded */
...
//                s->rtp_last_mv3_y = 0;
        ...
        cbpc = cbp & 3;
        if(s->dquant) cbpc+= 8; // see h263data.h for mb_type order in inter_MCBPC_code[]
        put_bits(&s->pb,
                 inter_MCBPC_bits[cbpc],
                 inter_MCBPC_code[cbpc]);
        ...
        if (s->rtp_save_mv) {
            s->rtp_last_mv0_x = pred_x;
            s->rtp_last_mv0_y = pred_y;
//            s->rtp_last_mv3_x =
//            s->rtp_last_mv3_y = 0;
...
//            s->rtp_last_mv3_y = 0;
        ...
        cbpc = cbp & 3;
        if (s->pict_type == I_TYPE) {
            if(s->dquant) cbpc+= 4; // see h263data.h for mb_type order in intra_MCBPC_code[]
            put_bits(&s->pb,
                intra_MCBPC_bits[cbpc],
                intra_MCBPC_code[cbpc]);
        } else {
        ...
        if (s->h263_aic) {
            put_bits(&s->pb, 1, 0); /* no AIC prediction */
        }
...
static void h263_encode_block(MpegEncContext * s, DCTELEM * block, int n)
{
    ...
    for (; i <= last_index; i++) {
        ...
        if (level) {
            ...
            if (code == rl->n) { // we just wrote the ESCAPE code
              if(s->h263_flv <= 1){
                ...
                else{ // modified quantization mode
                    put_bits(&s->pb, 8, 128);
...
static int h263_decode_gob_header(MpegEncContext *s)
{
    ...
    /* we have a GBSC, probably preceded by GSTUFF (up to 7 bits) */
...
//#define DEBUG 1
#ifdef DEBUG
    printf("Decode GOB header at MB %d, %06X\n", (s->mb_y * s->mb_width) + s->mb_x, show_bits(&s->gb, 24));
#endif
...
#ifdef DEBUG
    printf("GN %d (0,%d), GFID %d, Quant %d\n", s->gob_number, s->mb_y, gfid, s->qscale);
#endif
...
{
    int len, mb_num, mb_num_bits= av_log2(s->mb_num - 1) + 1, header_extension= 0;

    skip_bits(&s->gb, 16); /* drop the zeros */

    for(len=16; len<23; len++){
        if(get_bits1(&s->gb)) break;
    }
    ...
    if(s->shape != BIN_ONLY_SHAPE){
        ...
        if(qscale==0){
            printf("Error, video packet header damaged (qscale=0)\n");
        }else s->qscale= qscale;
    }
    ...
    if(header_extension){
        int time_increment, time_incr=0;
        ...
        skip_bits(&s->gb, 2); /* vop_coding_type */
...
int ff_h263_resync(MpegEncContext *s){
    ...
    if(s->codec_id==CODEC_ID_MPEG4) { // skip stuffing
        skip_bits1(&s->gb); // one '0'
        align_get_bits(&s->gb); // up to seven '1'
    }
    ...
    // OK, it's not where it's supposed to be ...
...
// printf("Inter(%d,%d) - dquant %d --> qscale %d, cbpy %d, cbpc %d", s->mb_x, s->mb_y, dquant, s->qscale, cbpy, cbpc);
...
// printf("Intra(%d,%d) - dquant %d --> qscale %d, cbpy %d, cbpc %d", s->mb_x, s->mb_y, dquant, s->qscale, cbpy, cbpc);
    ...
    */
    ...
    if (format != 7 && format != 6) {
        ...
        if (ufep == 1) {
            ...
            skip_bits1(&s->gb); /* Prevent start code emulation */
...
//printf("vo type:%d\n",s->vo_type);
    ...
    if ((s->vol_control_parameters=get_bits1(gb))) { /* vol control parameter */
        ...
        // set low delay flag only once so the smart (?) low delay detection won't be overriden
...
//        fprintf(stderr, "time_increment_resolution is illegal\n");
    ...
    if(s->time_increment_bits==0){
        printf("hmm, seems the headers aren't complete, trying to guess time_increment_bits\n");
