...
#define USES_LIST(a, list) ((a) & ((MB_TYPE_P0L0|MB_TYPE_P1L0)<<(2*(list)))) ///< does this mb use listX, note doesnt work if subMBs
    ...
    int32_t *mb_cmp_score;	///< Table for MB cmp scores, for mb decision 
    int b_frame_score;          /* */
} Picture;
...
/* the following codec id fields are deprecated in favor of codec_id */
    ...
    int mb_stride;             ///< mb_width+1 used for some arrays to allow simple addressng of left & top MBs without sig11
...
#define PICT_FRAME         3
    ...
    /* RTP friendly coding */
    /* ... these are explained in avcodec.h */
    int rtp_mode;
    int rtp_payload_size;
    void (*rtp_callback)(void *data, int data_size,
                         void *hdr, int hdr_size, void *priv_data);

    PutBitContext rtp_pb;    ///< used for payload headers
    uint8_t *rtp_pb_buffer;
...
int16_t *h263_pred_motion(MpegEncContext * s, int block, int *px, int *py);
