...
static inline void set_p_mv_tables(MpegEncContext * s, int mx, int my, int mv4)
{
    ...
    /* has already been set to the 4 MVs if 4MV is done */
    if(mv4){
...
#if 1 // this saves us a bit of clipping work and shouldnt affect compression in a negative way
...
{
    uint8_t *pix, *ppix;
    int sum, varc, vard, mx, my, range, dmin, xx, yy, norm1, i_score, p_score;
    ...
    yy = mb_y * 16;
    pix = s->new_picture.data[0] + (yy * s->linesize) + xx;
    ppix = ref_picture + ((yy+my) * s->linesize) + (xx+mx); // at this point (mx,my) are full-pell
    
    sum = s->dsp.pix_sum(pix, s->linesize);
    norm1 = s->dsp.pix_norm1(pix, s->linesize) - (((unsigned)(sum*sum)) >> 8) + 128;

    varc = (norm1 + 500) >> 8; // spatial complexity
    vard = (s->dsp.sse[0](NULL, pix, ppix, s->linesize) + 128) >> 8; // time complexity (i.e. change to previous picture)
    
//printf("varc=%4d (sum=%4d) vard=%4d mx=%2d my=%2d\n", varc, sum, vard, mx, my);
...
//printf("E%d %d %d %X %X %X\n", s->mb_width, mb_x, mb_y, (int)s, (int)s->mb_var, (int)s->mc_mb_var);

    if (vard <= 64 || vard < varc){ // no change from previous picture or high spatial complexity
            s->scene_change_score+= ff_sqrt(vard) - ff_sqrt(varc);
    }else{ // much different from previous picture, scene change?
        s->scene_change_score += 32*s->qscale;
// printf("(%2d,%2d) varc=%4d (sum=%4d) vard=%4d mv=(%2d,%2d)\n", mb_x, mb_y, varc, sum, vard, mx, my);
    }

    if(s->avctx->mb_decision != FF_MB_DECISION_SIMPLE){ // one or more MB type(s) possible
        mb_type= 0;
        if (varc*2 + 200 > vard){
        ...
        // at this point (mx,my) are half-pell
        ...
        if((s->avctx->me_sub_cmp != s->avctx->mb_cmp)
           && !s->me.skip)
        ...
        p_score = pic->mb_cmp_score[s->mb_stride * mb_y + mb_x] = dmin;
        i_score = norm1 + 500 - 24*s->qscale*s->qscale;

        if(i_score < p_score){
            mb_type= MB_TYPE_INTRA;
// printf("(%2d,%2d) is now INTRA, iscore %4d, pscore %4d\n", mb_x, mb_y, i_score, p_score);
        }
    }
