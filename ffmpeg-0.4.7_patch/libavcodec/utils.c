...
int avcodec_default_get_buffer(AVCodecContext *s, AVFrame *pic){
    ...
    if(buf->base[0]){
        ...
        total_size = 0;
        for(i=0; i<3; i++){
            const int h_shift= i ? h_chroma_shift : 0;
            const int v_shift= i ? v_chroma_shift : 0;
            int plane_size;

            pic->linesize[i]= pixel_size*w>>h_shift;
            plane_size= (pic->linesize[i]*h>>v_shift);

            buf->data[i] = buf->base[i] = (uint8_t *)total_size;
            if(!(s->flags&CODEC_FLAG_EMU_EDGE))
                buf->data[i] += (pic->linesize[i]*EDGE_WIDTH>>v_shift) + (EDGE_WIDTH>>h_shift);
        
            total_size += plane_size;
        }

        ptr= av_malloc(total_size + 16); //FIXME, see += 16 in load_input_picture()
        if(ptr==NULL) return -1;
        memset(ptr, 128, total_size);
